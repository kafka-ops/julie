...
public class Configuration {
  ...
  public Properties asProperties(String prefix) {
    ...
    if (cliParams.get(BROKERS_OPTION) != null) {
  ...
  private void validateBrokersConfig() throws ConfigurationException {
    ...
    if (cliParams.get(BROKERS_OPTION) == null && !existServersAsConfig) {
      String msg =
          String.format(
              "Either the CLI option %s or the configuration %s should be specified",
              BROKERS_OPTION, AdminClientConfig.BOOTSTRAP_SERVERS_CONFIG);
  ...
  public boolean isQuiet() {
    return Boolean.parseBoolean(cliParams.getOrDefault(QUIET_OPTION, "false"));
  }

  public boolean doValidate() {
    return Boolean.parseBoolean(cliParams.getOrDefault(VALIDATE_OPTION, "false"));
  }
